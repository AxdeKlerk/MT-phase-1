...
def start_payment(request):
    ...
    try:
        payment_intent_obj, created = PaymentIntent.objects.get_or_create(
...
def phase1_report(request):
    ...
    for condition in ["poster", "card", "unknown"]:
        ...
        # Use tip_amount (the pure tip, excluding processing fees) for accurate averages
        avg_tip = Payment.objects.filter(
            processor_id__in=successes.values_list(
                "stripe_payment_intent_id",
                flat=True
            )
        ).aggregate(avg=Avg("tip_amount"))["avg"]
